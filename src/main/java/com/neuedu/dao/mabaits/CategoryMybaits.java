...
//        SqlSession session  = MyBatisFactory.getSqlSession();
      ...
      //  parent_id=?,cname=?,status=?,sort_order=?,create_time=?,update_time=? where id=?
        ...
        sqlSession.update("com.neuedu.entity.Category.updateCategory",map);
        return true;
    }
...
//        SqlSession session  = MyBatisFactory.getSqlSession();
    ...
    public PageModel<Category> findCategoryByPage(int pageNo, int pageSize) {
        //step1:查询总记录数，计算总共页数
//        SqlSession session  = MyBatisFactory.getSqlSession();
        int    totalcount= (Integer) sqlSession.selectOne("com.neuedu.entity.Category.findTotalCount");
        //step2：查询某页数据
        Map<String,Object> map=new HashMap<String, Object>();
        map.put("offest",(pageNo-1)*pageSize);
        map.put("pageSize",pageSize);
        List<Category> Categorys= sqlSession.selectList("com.neuedu.entity.Category.findcategoryPage",map);
...
//        SqlSession session  = MyBatisFactory.getSqlSession();
        List<Category> list=  sqlSession.selectList("com.neuedu.entity.Category.findAllparentname");
        return list;
    }
...
//        SqlSession session  = MyBatisFactory.getSqlSession();
        List<Category> list=      sqlSession.selectList("com.neuedu.entity.Category.findAllsonname",parentname);
        return list;
    }
}
